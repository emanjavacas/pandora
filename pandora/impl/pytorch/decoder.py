...
class AttentionalDecoder(nn.Module):
    ...
    # def beam_batch(self, token_out, context_out, token_context, lemma_out,
    #                bwidth=5, max_seq_len=20):
    #     bos, eos = self.char_dict[BOS], self.char_dict[EOS]
    #     # use last encoder state as hidden, add num_layers * num_dirs singleton
    #     init_hidden = token_context[-1].unsqueeze(0)
    #     batch = token_out.size(0)
    #     prev = token_out.data.new([bos]).expand(batch).long()
    #     # append token context as extra input sequence step
    #     # token_context: (inp_seq + 1 x batch x hidden)
    #     token_context = torch.cat([token_context, context_out.unsqueeze(0)])

    #     def score_fn(inp, hidden):
    #         inp, dim0 = Variable(inp, volatile=True), inp.size(0)
    #         btoken_context = None  # eventually expand token_context along beam
    #         if hidden is not None:
    #             # not first step, broadcast along batch
    #             hidden = utils.broadcast(hidden, dim0, 1)
    #             btoken_context = utils.broadcast(token_context, dim0, 1)
    #         else:
    #             hidden = init_hidden
    #             btoken_context = token_context
    #         # (batch (* bwidth) x emb_dim)
    #         prev_emb = self.embeddings(inp).unsqueeze(0)
    #         # output: (1 x batch x hidden)
    #         _, hidden = self.rnn(prev_emb, hidden)
    #         # context: (1 x batch x hidden)
    #         context, _ = self.attn(hidden, btoken_context)
    #         # context: (1 x batch x hidden + emb_dim)
    #         # -> (batch x hidden + emb_dim)
    #         context = torch.cat([prev_emb, context], 2).squeeze(0)
    #         # (batch (* bwidth) x vocab)
    #         log_prob = F.log_softmax(self.proj(context))
    #         return log_prob.view(batch, -1, self.char_vocab).data, hidden

    #     def output_fn(hidden, source_beam):
    #         return utils.multiple_index_select(
    #             hidden.view(batch, bwidth, -1), source_beam
    #         ).view(1, batch * bwidth, -1)

    #     # (batch x beam x seq_len)
    #     decoded, _ = beam_search(
    #         score_fn, output_fn, prev, eos, bwidth, max_seq_len)
